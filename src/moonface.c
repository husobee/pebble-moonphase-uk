...
static void update_time()
{
	...
	strftime(date_buffer, sizeof("01/01/70"), "%d/%m/%y", tick_time);
...
static void canvas_update_proc(Layer *this_layer, GContext *ctx)
{
	...
	// set background
	graphics_fill_rect(ctx, GRect(0, 0, bounds.size.w, bounds.size.h), 0,
			   GColorBlack);
	...
	// we are going to make strokes line by line down the Y axis to make our
	// dark side
	time_t now = time(NULL);
	struct tm *t = localtime(&now);

	double phase = approximate_moon_phase(
	    julian_date(t->tm_mday, t->tm_mon + 1, t->tm_year + 1900));
	int y;
	for (y = -44; y <= 44; y++) {
